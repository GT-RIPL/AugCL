...
class DArQMiddle(SAC):
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, L=None, step=None):
        ...
        for _ in range(self.k):
            ...
            x_encoder = F.dropout2d(
                self.critic.encoder(augmentations.random_shift(obs)),
                p=p_dropout,
            )
            current_Q1 = self.critic.Q1(x_encoder, action)
            current_Q2 = self.critic.Q2(x_encoder, action)
