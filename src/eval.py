...
def main(args):
    ...
    if args.eval_mode == "distracting_cs":
        results_fp = os.path.join(
            ...
            args.eval_mode
            + "_"
            + str(args.distracting_cs_intensity).replace(".", "_")
            + ".pt",
        )
    else:
    ...
    assert not os.path.exists(
        results_fp
    ), f"{args.eval_mode} results already exist for {work_dir}"
    ...
    if args.algorithm == "pad":
        ...
        adapt_reward = evaluate(
            env, agent, video, args.eval_episodes, args.eval_mode, adapt=True
        )
        print("Adapt reward:", int(adapt_reward))

    # Save results
    torch.save(
        {"args": args, "reward": reward, "adapt_reward": adapt_reward}, results_fp
    )
